...
export function CallNavStatus({ docked = false }: CallNavStatusProps) {
  ...
  // Build member data for the voice channel user list.
  // Filter out SFU/bot entries: real Matrix user IDs always start with '@'
  // and contain a ':' (server part). Base64 focus identities do not.
  const memberListData = callMembers.filter((userId) =>
    userId.startsWith('@') && userId.includes(':')
  ).map((userId) => {
